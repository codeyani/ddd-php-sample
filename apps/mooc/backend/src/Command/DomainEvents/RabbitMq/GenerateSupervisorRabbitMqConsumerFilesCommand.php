...
namespace Codeyani\Apps\Mooc\Backend\Command\DomainEvents\RabbitMq;
...
#[AsCommand(
	name: 'codeyani:domain-events:rabbitmq:generate-supervisor-files',
...
final class GenerateSupervisorRabbitMqConsumerFilesCommand extends Command
{
	...
	{
		return <<<EOF
            [program:codeyani_{queue_name}]
            command      = {path}/apps/mooc/backend/bin/console codeyani:domain-events:rabbitmq:consume --env=prod {queue_name} {events_to_process}
